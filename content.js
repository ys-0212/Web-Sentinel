...
// Allowlist of reputable roots where CDN-heavy patterns are normal
const ALLOWLIST_SUFFIXES = [
    'google.com','googleapis.com','gstatic.com','youtube.com','ytimg.com',
    'microsoft.com','azureedge.net','live.com','office.com','windows.net',
    'cloudflare.com','cdn.cloudflare.net','apple.com','akamaized.net',
    'facebook.com','fbcdn.net','instagram.com','whatsapp.com',
    'amazon.com','awsstatic.com','github.com','githubassets.com'
];

function isAllowlisted(hostname){
    try {
        if (!hostname) return false;
        return ALLOWLIST_SUFFIXES.some(function(sfx){
            return hostname === sfx || hostname.endsWith('.' + sfx);
        });
...
function runPhishingScan(){
    ...
    // Neutralize cross-domain ratio features on allowlisted roots only (features 10-12)
    if (isAllowlisted(window.location.hostname)){
        testdata[9] = 0;  // isImgFromDifferentDomain
        testdata[10] = 0; // isAnchorFromDifferentDomain
        testdata[11] = 0; // isScLnkFromDifferentDomain
    }
